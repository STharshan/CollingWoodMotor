...
const StatusCell = ({ value }) => {
  if (value === true) return <div className="bg-green-600 h-6 w-6 rounded-sm mx-auto" />;
  if (value === false) return <div className="bg-red-600 h-6 w-6 rounded-sm mx-auto" />;
  return <div className="text-sm text-white text-center">N/A</div>;
};
...
const ServiceTable = () => {
  return (
    //top table
    <section className="bg-white text-white py-10 px-2 sm:px-6">
      {/* Eco Pro Box */}
      <div className="bg-black text-white text-center mx-auto py-8 px-4 rounded-md shadow-md mb-12 w-full sm:w-1/2">
        <h3 className="text-lg sm:text-xl font-bold mb-2">
          Why not upgrade to Eco Pro and take your performance to the next level!?
        </h3>
        <p className="text-3xl sm:text-4xl font-extrabold text-[#0093FF] mb-4">£29.95</p>
      ...
      <div className="max-w-5xl mx-auto w-full bg-black rounded-md pt-10 px-4">
        {/* Table Header */}
        <div className="grid grid-cols-3 text-center text-base sm:text-lg font-bold mb-4 bg-[#0093FF] rounded-md overflow-hidden max-w-4xl mx-auto">
        ...
        <div className="space-y-2 max-w-4xl mx-auto">
          {services.map((service, index) => (
            <div
              key={index}
              className="grid grid-cols-3 text-sm sm:text-base bg-[#1a1a1a] border border-gray-700 rounded-md"
            >
              <div className="py-3 px-2">{service.name}</div>
              <div className="py-3">
      ...
      <div className="max-w-xl mx-auto my-10 border border-black shadow-md rounded-md">
        {/* Header */}
        <div className="bg-blue-500 text-white text-center font-semibold py-3 px-4 text-base sm:text-lg underline">
          Please note following
        </div>

        {/* Content */}
        <div className="bg-black text-white text-sm sm:text-base">
          {notes.map((item, index) => (
            <div
              key={index}
              className="border-t border-blue-500 py-4 px-4 text-center"
            >
              {item.text}
            </div>
          ))}
